...
public class TreeGridDeclarativeTest
    ...
    public void dataSerialization() throws InstantiationException,
        ...
        HierarchyDataBuilder<Person> builder = HierarchyData.builder();
        builder.addItems(null, person1, person4, person5);
        builder.addItems(person1, person2, person3);
        builder.addItem(person5, person6);
        builder.addItem(person6, person7);

        grid.addColumn(Person::getFirstName).setCaption("First Name");
        grid.addColumn(Person::getLastName).setId("id").setCaption("Id");

        grid.setHierarchyColumn("id");
        grid.setDataProvider(new InMemoryHierarchicalDataProvider<>(builder));
        ...
        TreeGrid<Person> readGrid = testRead(design, grid);
        ...
        Assert.assertEquals(2, readGrid.getDataProvider()
                .size(new HierarchicalQuery<>(null, person1)));
        Assert.assertEquals(1, readGrid.getDataProvider()
                .size(new HierarchicalQuery<>(null, person5)));
        Assert.assertEquals(1, readGrid.getDataProvider()
                .size(new HierarchicalQuery<>(null, person6)));
